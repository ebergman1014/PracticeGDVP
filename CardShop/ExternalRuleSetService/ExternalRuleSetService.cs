...
namespace Microsoft.Samples.Rules.ExternalRuleSetService
{
    public class ExternalRuleSetService : WorkflowRuntimeService
    {
        public ExternalRuleSetService()
        {
            ...
            {
                if (string.CompareOrdinal(connectionStringSettings.Name, "RuleSetStore") == 0)
                    connectionString = connectionStringSettings.ConnectionString;
            }

            if (connectionString == null)
                throw new ConfigurationErrorsException("SQL connection string not available (should be provided in the config file).");
        }
